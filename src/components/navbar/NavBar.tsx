...
const NavBar = () => {
  ...
  const handleClickOutside = (event: MouseEvent) => {
    ...
    if (
      (busDropdownRef.current && !busDropdownRef.current.contains(event.target as Node)) &&
      (timeTableDropdownRef.current && !timeTableDropdownRef.current.contains(event.target as Node))
    ) {
  ...
  useEffect(() => {
    ...
    // Automatically close dropdown after 8 seconds
